...
{
    // DPI awareness - must call before any UI/cursor operations
    ...
    [DllImport("user32.dll")] public static extern bool DrawIconEx(IntPtr hdc, int x, int y, IntPtr hIcon, int cxWidth, int cyWidth, uint istepIfAniCur, IntPtr hbrFlicker, uint diFlags);
...
public class DXGISender : IDisposable
{
    ...
    {
        // Set per-monitor DPI aware so cursor coords match physical pixels
        try { N.SetProcessDpiAwareness(2); } catch { N.SetProcessDPIAware(); }
        try
        {
            // JPEG encoder
            ...
            VT.F<VT.EnumOutputsD>(adapter, 7)(adapter, 1, out output0); // DISPLAY2
            DXGI_OUTPUT_DESC outDesc;
            VT.F<VT.GetDescD>(output0, 7)(output0, out outDesc);
            screenLeft = outDesc.Left;
            screenTop = outDesc.Top;
            Console.WriteLine("Display offset: " + screenLeft + "," + screenTop);
        ...
        try
        {
            ...
            if (hr == 0)
            {
                ...
                if (hr == 0)
                {
                    ...
                    // Draw mouse cursor onto fullBmp
                    // With DPI awareness, cursor coords are in physical pixels
                    N.CURSORINFO ci = new N.CURSORINFO();
                    ci.cbSize = Marshal.SizeOf(typeof(N.CURSORINFO));
                    if (N.GetCursorInfo(ref ci) && ci.flags == 1 && ci.hCursor != IntPtr.Zero)
                    {
                        int cx = ci.x - screenLeft;
                        int cy = ci.y - screenTop;
                        if (cx >= 0 && cx < texW && cy >= 0 && cy < texH)
                        {
                            N.ICONINFO ii;
                            N.GetIconInfo(ci.hCursor, out ii);
                            // Draw cursor via GDI+ for smooth scaling
                            int cursorSize = 64;
                            using (var cursorBmp = new Bitmap(cursorSize, cursorSize, PixelFormat.Format32bppArgb))
                            {
                                // Draw icon at native size into temp bitmap
                                using (var cg = Graphics.FromImage(cursorBmp))
                                {
                                    IntPtr hdc = cg.GetHdc();
                                    N.DrawIconEx(hdc, 0, 0, ci.hCursor, cursorSize, cursorSize, 0, IntPtr.Zero, N.DI_NORMAL);
                                    cg.ReleaseHdc(hdc);
                                }
                                // Composite onto fullBmp with high quality interpolation
                                using (var g = Graphics.FromImage(fullBmp))
                                {
                                    g.InterpolationMode = System.Drawing.Drawing2D.InterpolationMode.HighQualityBicubic;
                                    g.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.AntiAlias;
                                    g.DrawImage(cursorBmp, cx - ii.xHotspot, cy - ii.yHotspot, cursorSize, cursorSize);
                                }
                            }
                        }
                    }
